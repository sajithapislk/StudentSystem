...
namespace Project1.DAO
{
    class TeacherDAO
    {
        ...
        public bool insert(string firstName, string lsatName, string dob, string gender, string address, string tp)
        {
            using (SqlConnection connection = new SqlConnection(db.GetConnection()))
            {
                try
                {
                    ...
                    string query = "INSERT INTO teachers(first_name,last_name,date_of_birth,gender,address,home_tp) VALUES(@firstName,@lsatName,@dob,@gender,@address,@tp)";
        ...
        public bool update(string id, string firstName, string lsatName, string dob, string gender, string address, string tp)
        {
            using (SqlConnection connection = new SqlConnection(db.GetConnection()))
            {
                try
                {
                    ...
                    string query = "UPDATE teachers SET first_name=@firstName,last_name=@lsatName,date_of_birth=@dob,gender=@gender,address=@address,home_tp=@tp WHERE id=@id";
        ...
        public SqlDataReader TeacherInfo(string id)
        {
            using (SqlConnection connection = new SqlConnection(db.GetConnection()))
            {
                try
